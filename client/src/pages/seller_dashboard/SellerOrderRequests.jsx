...
function SellerOrderRequests() {
  ...
  const getOrders = async () => {
    ...
    // Set the status to "delivered" directly
    const updatedData = orderedData.map((order) => ({
      ...order,
      status: "delivered", // Directly set status to "delivered"
    }));
              ...
              <tbody>
                {data.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">{item.userId.contact}</td>
